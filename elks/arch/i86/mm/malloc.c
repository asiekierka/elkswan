...
segment_s * seg_dup_bank (segment_s * src, unsigned char bank)
{
    unsigned int size = src->size;
    unsigned int flags = src->flags;

    unsigned char cur_bank = bank_get_current();
    if (bank == cur_bank)
        return seg_dup(src);
    ...
    if (dst) {
        // TODO: Optimize.
        const unsigned char __far *srcp = _MK_FP(src->base, 0);
        unsigned char __far *destp = _MK_FP(dst->base, 0);
